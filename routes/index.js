  ...
  request(url, function(err,response,body){
    if(err){
      ...
      res.render("index", {weather: null, error: 'Error, please try again'});
            ...
            // wind speed
  					currWeather['windSpeed']		  = Math.round((weather.wind.speed)*1.61);
            ...
            // current temperature
            var response = "Current Weather: "+currWeather['currTemp']+"\xB0 C and "+currWeather['description'];
            if(currWeather['windSpeed']>0) {// if there's wind, add a wind description to the response
            response = response+" with winds out of the "+windShortNames[currWeather['windCompass']]+" at "+currWeather['windSpeed']+" km per hour";
            }
